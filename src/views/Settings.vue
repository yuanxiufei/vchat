    ...
    <div class="grid grid-cols-12 gap-6">
      ...
      >
        ...
        >
          ...
          <DialogRoot v-model:open="showImg">
            <DialogPortal to="body">
              <DialogOverlay class="fixed inset-0 bg-black/60" />
              <DialogContent
                class="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[2000] bg-white rounded-xl shadow-lg p-2 w-[95vw] max-w-[1200px]"
              >
                <img
                  :src="previewImg"
                  alt="preview"
        ...
        <div v-if="activeTab === 'models'" class="space-y-6">
            ...
            <DialogRoot v-model:open="showAdd">
              ...
              <DialogPortal to="body">
                ...
                >
                  ...
                  <p id="add-model-desc" class="text-xs text-gray-500 mb-3">
                    请填写必要参数后点击添加
                  </p>
                  <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
                    <div>
                      ...
                      <SelectRoot v-model="newType">
                        <SelectTrigger
                          class="w-full h-9 border rounded-md px-3 flex items-center justify-between"
                        >
                          <SelectValue :placeholder="t('select_type')" />
                        </SelectTrigger>
                        <SelectPortal to="body">
                          <SelectContent
                            class="bg-white rounded-md shadow-md z-[1000] border"
                          >
                      ...
                      <div v-if="typeExists" class="text-xs text-red-600 mt-1">
          ...
          >
            ...
            <template v-for="key in openaiCustomKeys" :key="key">
              ...
              >
                <summary class="cursor-pointer font-medium">
                  ...
                  <span v-if="!hasModelsFor(key)" class="text-xs text-gray-500"
                    >（未配置模型）</span
                  >
                </summary>
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-3 mt-3">
            ...
            <template v-for="key in dashscopeCustomKeys" :key="key">
              ...
              >
                <summary class="cursor-pointer font-medium">
                  {{ providers[key].title || t("dashscope_title") }}（{{
                    key
                  }}）
                  <span v-if="!hasModelsFor(key)" class="text-xs text-gray-500"
                    >（未配置模型）</span
...
import imgOpenai from "@/styles/images/moxing/openaI.png";
import imgDashscope from "@/styles/images/moxing/tongyiqianwen.png";
import imgQianfan from "@/styles/images/moxing/wenxinyiyan.png";
import imgDeepseek from "@/styles/images/moxing/deepseek.png";
import imgClaude from "@/styles/images/moxing/claude.png";
import imgOpenaiEn from "@/styles/images/moxing/openai-en.png";
import imgDashscopeEn from "@/styles/images/moxing/tongyiqianwen-en.png";
import imgQianfanEn from "@/styles/images/moxing/wenxinyiyan-en.png";
import imgDeepseekEn from "@/styles/images/moxing/deepseek-en.png";
import imgClaudeEn from "@/styles/images/moxing/claude-en.png";
const toastMsg = ref("");
function showToast(key: string) {
  toastMsg.value = t(key);
  setTimeout(() => {
    toastMsg.value = "";
  }, 2000);
}
...
function openDoc(type: string) {
  ...
  const map: Record<string, string> = {
    ...
    openai_compat:
      "https://help.aliyun.com/zh/model-studio/install-sdk?spm=a2c4g.11186623.help-menu-2400256.d_2_0_2.2f7d11fcn5ArWa&scm=20140722.H_2712193._.OR_help-T_cn~zh-V_1#77db53c233mol",
  };
  const url = map[type] || "https://dashscope.aliyun.com/";
  (window as any).electronAPI.openExternal(url);
}
// 获取 API Key 的控制台链接
function openKey(type: string) {
  const keyMap: Record<string, string> = {
    openai: "https://platform.openai.com/api-keys",
    deepseek: "https://platform.deepseek.com/",
    claude: "https://console.anthropic.com/",
    qianfan: "https://console.bce.baidu.com/qianfan/ais/console/",
    dashscope: "https://dashscope.console.aliyun.com/",
  };
  const url = keyMap[type];
...
function normalizeProviders(p: any) {
  ...
  if (p && typeof p === "object") {
    Object.keys(p).forEach((k) => {
      if (/^(dashscope|qianfan|openai|deepseek|claude)_/.test(k))
        base[k] = p[k];
    });
  }
  return base;
...
async function addProvider() {
  ...
  if (k === "qianfan") {
    ...
    p[key] = {
      ...
      models: modelsList.length
        ? modelsList
        : cur.models || ["claude-3-5-sonnet-latest"],
